...
//$search_key1 = strip_input('inputadm', $_REQUEST['search_key1']);//
$search_key2 = strip_input('inputadm', $_REQUEST['search_key2']);//搜尋關鍵字，陣列
$search_key2_value = "";
//把有值的選項抓出來
foreach($search_key2 AS $value2){
    if($value2 != ""){
        $search_key2_value = $value2;
    }
}
$search_type2 = strip_input('inputadm', $_REQUEST['search_type2']);//搜尋方式二，陣列
//把有值的那個選項抓出來
$search_type2_value="";//搜尋方式二，值
foreach($search_type2 AS $value3){
    if($value3 !== ""){
        $search_type2_value = $value3;
    }
}
...
$tpl->gotoBlock('_ROOT');
//輸出分類
$strSQL1 = "select '1' as list_id, '商品銷售數量查詢' as list_title
";
...
$strSQL_sold ="
    SELECT 
        ...
        order_listdetail.ord_qty AS sold_qty, 
    ...
    AND (order_listdetail.prod_num LIKE '%Y340%' OR order_listdetail.prod_num LIKE '%Y341%')
...
//搜尋條件的SQL文
$strSQL_selected="";
switch($search_type2_value){
    case 'like_prod_namesub':
        $strSQL_selected .= "order_listdetail.prod_namesub LIKE '%".$search_key2_value."%'";
        $search_type_str="商品副標包含";
        break;
    case 'like_prod_name':
        $strSQL_selected .= "order_listdetail.prod_name LIKE '%".$search_key2_value."%'";
        $search_type_str="商品名稱包含";
        break;
    case 'sameAs_prod_num':
        $strSQL_selected .= "order_listdetail.prod_num ='".$search_key2_value."'";
        $search_type_str="與商品編號一致";
        break;
    case 'like_prod_num':
        $strSQL_selected .= "order_listdetail.prod_num LIKE '%".$search_key2_value."%'";
        $search_type_str="商品編號包含";
        break;
}
...
if($search_type == ""){
    ...
    while(!$resSold01->EOF){
        ...
        $tpl->assign('prod_namesub', $resSold01->fields['prod_namesub']);//商品名稱副標
...
$strSQL_sold_total = "
    ...
    ORDER BY order_listdetail.prod_num ASC
";
...
if($search_type ==""){
    ...
    //筆數&分頁
    /*
    $total_order = GetQueryValueCount1($strSQL_sold_total, 'value');//計算筆數
    pageft($total_order, $page_splt01);
    $resSold_total_01 = SqlQueryLimit1($strSQL_sold_total, $displaypg, $firstcount);
    $tpl->assign('pagenav_total', $pagenav);
    $tpl->assign('pagejump01', $pagejump01);
    $tpl->assign('page_splt01_add', ($page_splt01 * ($page - 1)));
    $tpl->assign('page_splt01', $page_splt01);
    $tpl->assign('page', $page);
    */
    ...
    $resSold_total_01 = SqlQuery1($strSQL_sold_total);
    $k = 0;//用來判斷奇偶數的數
    while(!$resSold_total_01->EOF){
        $k++;
        //指派給HTML，搭配註釋標籤
        $tpl->newBlock('Table_row_total');
        //一行與另一行顏色不同
        $tpl->assign('rowid02ck', ($k % 2 == 1) ? 'class="odd"' : '');
        
        //$tpl->assign('orderid', $resSold_total_01->fields['orderid']);//訂單編號
        $tpl->assign('prod_num_total', $resSold_total_01->fields['prod_num']);//商品編號
        $tpl->assign('sold_qty_total', $resSold_total_01->fields['sold_qty']);//賣出數量
        //$tpl->assign('add_date', $resSold_total_01->fields['add_date']);//新增時間
        $tpl->assign('ord_type_total', $resSold_total_01->fields['ord_type']);//商品類型
        $tpl->assign('ord_typestr_total', $resSold_total_01->fields['ord_typestr']);//商品類型說明
        $tpl->assign('prod_name_total', $resSold_total_01->fields['prod_name']);//商品名稱
        $tpl->assign('prod_price_total', $resSold_total_01->fields['prod_price']);//商品單價
        //$tpl->assign('gcname', $resSold_total_01->fields['gcname']);//訂購姓名
        //$tpl->assign('add_ipaddress', $resSold_total_01->fields['add_ipaddress']);//新增位址
        $tpl->assign('prod_namesub_total', $resSold_total_01->fields['prod_namesub']);//商品名稱副標
        
        $resSold_total_01->MoveNext();
    }
